 ...
 */
@Component
public class ClearMerChantRecordClient extends BaseClient{
	...
	public String batchQueryRecord(ClearMerchantRecordModel clearMerchantRecordModel) {
		ClearMerchantRecordService.Client client = this.getClient();
		try {
			
			String queryByConditions = client.getClearMerchantRecordMessage(clearMerchantRecordModel);
			
			return queryByConditions;
			
		} catch (Exception e) {
			logger.error("数据库没有查询到数据等待下一次执行");
		} finally {
			this.close();
		}
		return null;
