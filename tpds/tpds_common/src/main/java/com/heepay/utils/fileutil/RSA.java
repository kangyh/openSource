...
public class RSA {
	...
	public static byte[] encryptByPrivateKey(byte[] data, String privateKey) throws Exception {
		System.out.println("进入加密流程***********");
		byte[] keyBytes = Base64Utils.decode(privateKey);
		System.out.println("进入加密流程******1111*****");
		PKCS8EncodedKeySpec pkcs8KeySpec = new PKCS8EncodedKeySpec(keyBytes);
		KeyFactory keyFactory = KeyFactory.getInstance(KEY_ALGORITHM);
		System.out.println("进入加密流程*****22222233******" + keyBytes);
		Key privateK = keyFactory.generatePrivate(pkcs8KeySpec);
		Cipher cipher = Cipher.getInstance(keyFactory.getAlgorithm());
		System.out.println("进入加密流程*****333333333333******" + privateK + "******" + Cipher.ENCRYPT_MODE);
		cipher.init(Cipher.ENCRYPT_MODE, privateK);
		int inputLen = data.length;
		System.out.println("进入加密流程*****444444444444******");
		ByteArrayOutputStream out = new ByteArrayOutputStream();
		int offSet = 0;
		byte[] cache;
		int i = 0;
		// 对数据分段加密
		while (inputLen - offSet > 0) {
			// System.out.println("进入加密流程****5555555555*******");
			if (inputLen - offSet > MAX_ENCRYPT_BLOCK) {
				cache = cipher.doFinal(data, offSet, MAX_ENCRYPT_BLOCK);
			} else {
				cache = cipher.doFinal(data, offSet, inputLen - offSet);
	 ...
	 * @throws Exception
	 */
	public void saveKey(Map<String, Object> keyMap) throws Exception {
		String privateKey = RSA.getPrivateKey(keyMap);
		String publicKey = RSA.getPublicKey(keyMap);
		PrintWriter pw1 = new PrintWriter(new FileOutputStream("D:/"));
		PrintWriter pw2 = new PrintWriter(new FileOutputStream("D:/"));
		pw1.print(privateKey);
		pw2.print(publicKey);
		pw1.close();
		pw2.close();
	}
