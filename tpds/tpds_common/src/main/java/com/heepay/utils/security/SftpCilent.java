 ...
 * 描 述：远程文件下载客户端
 *
 * 创 建 者： wangl 创建时间： 2017年1月24日下午3:51:55 创建描述：
 * 
 * 修 改 者： 修改时间： 修改描述：
 * 
 * 审 核 者： 审核时间： 审核描述：
...
public class SftpCilent {
	...
	public File getFile(String host, String name, String passsword, int port, String filePath) {
		...
		File file = null;
		ChannelSftp sftp = ftp.connect(host, port, name, passsword,null);
		try {
	 ...
	 * @创建人：wangl
	 */
	public ChannelSftp connect(String host, int port, String username, String password,String miPath) {
		...
		try {
			...
			/*try {
				csftp.mkdir(miPath);
				logger.info("创建加密文件文件夹{}",miPath);
			} catch (SftpException e) {
				logger.debug("存放密文的文件夹已存在{}",e);
			}*/
		} catch (JSchException e) {
			e.printStackTrace();
		}
	 ...
	 *            本地带文件名的绝对路径
	 * 
	 * @方法说明：文件上传到远程服务器 @时间：14 Feb 201710:10:19
	 * @创建人：wangl
	 */
