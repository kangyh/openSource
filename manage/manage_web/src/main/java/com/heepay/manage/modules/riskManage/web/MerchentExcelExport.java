...
@Component
public class MerchentExcelExport {
	...
	public void exportExcel(String fileName, String[] headers, List<Map<String, Object>> dataset,
		...
		try {
	        ...
	        for(Map<String,Object> valMap: dataset) {
	            ...
	            for (short i = 0; i < showField.length; i++) {
					...
					if(null != valMap.get(showField[i])){
						if(StringUtils.equals(showField[i], "status")){
							cell.setCellValue(RiskStatus.labelOf(valMap.get(showField[i]).toString()));
						//freezeType
						}else if(StringUtils.equals(showField[i].toString(), "freezeType")){
							cell.setCellValue(RiskFreezeType.labelOf(valMap.get(showField[i]).toString()));
						//freezeRemark
						}else if(StringUtils.equals(showField[i].toString(), "freezeRemark")){
							cell.setCellValue(RiskFreezeRemark.labelOf(valMap.get(showField[i]).toString()));
						//createTime //处理时间
	        ...
	        workbook.write(out);
	        logger.info("商户冻结下载结束");
		}catch (IOException e) {
			logger.error("商户冻结下载错误",e);
        } catch (Exception e) {
        	logger.error("商户冻结下载错误",e);
		}finally {
        	try {
