...
<body>
	<ul class="nav nav-tabs">
		<li class="active"><a href="${ctx}/payment/statisticsOutInAmountRecord/">分类交易数据统计列表</a></li>
		<shiro:hasPermission name="payment:statisticsTransRecord:edit"><li><a href="${ctx}/payment/statisticsOutInAmountRecord/form">交易数据统计添加</a></li></shiro:hasPermission>
	</ul>
	<form:form id="searchForm" modelAttribute="statisticsTransRecord" action="${ctx}/payment/statisticsOutInAmountRecord/" method="post" class="breadcrumb form-search">
	...
	<table id="contentTable" class="table table-striped table-bordered table-condensed">
		...
		<tbody>
		<c:forEach items="${outList}" var="statisticsTransRecord">
			<tr>
		...
		<tr>
				...
				<td style="text-align: right">
					 <fmt:formatNumber value="${outTotalAmount}" pattern="￥#,##0.00" /> 
					
				</td>
				<td>
					<fmt:formatNumber value="${outSucessCount/outTotalCount}" pattern="0.00%" /> 
				</td>
				<td>
					${outSucessCount }
				</td>
				<td style="text-align: right">
					 <fmt:formatNumber value="${outSucessAmount}" pattern="￥#,##0.00" /> 
				</td>
				<td>
					
					 <fmt:formatNumber value="${outSucessCount/((outFailCount+outSucessCount) eq 0?1:(outFailCount+outSucessCount))}" pattern="0.00%" /> 
				</td>
				<td>
					${outFailCount}
				</td>
				<td style="text-align: right">
					<fmt:formatNumber value="${outFailAmount}" pattern="￥#,##0.00" /> 
					
				</td>
				<td>
					<fmt:formatNumber value="${outFailCount/((outFailCount+outSucessCount) eq 0?1:(outFailCount+outSucessCount))}" pattern="0.00%" /> 
				</td>
	...
	<table id="contentTable" class="table table-striped table-bordered table-condensed">
		...
		<tbody>
		<c:forEach items="${inList}" var="statisticsTransRecord">
			<tr>
				<td>
					${fns:getDictLabel(statisticsTransRecord.productCode, 'ProductCodeType', '')}
		...
		<tr>
				...
				<td style="text-align: right">
					 <fmt:formatNumber value="${inTotalAmount}" pattern="￥#,##0.00" /> 
					
				</td>
				<td>
					<fmt:formatNumber value="${inSucessCount/inTotalCount}" pattern="0.00%" /> 
				</td>
				<td>
					${inSucessCount }
				</td>
				<td style="text-align: right">
					 <fmt:formatNumber value="${inSucessAmount}" pattern="￥#,##0.00" /> 
				</td>
				<td>
					
					 <fmt:formatNumber value="${inSucessCount/((inFailCount+inSucessCount) eq 0?1:(inFailCount+inSucessCount))}" pattern="0.00%" /> 
				</td>
				<td>
					${inFailCount}
				</td>
				<td style="text-align: right">
					<fmt:formatNumber value="${inFailAmount}" pattern="￥#,##0.00" /> 
					
				</td>
				<td>
					<fmt:formatNumber value="${inFailCount/((inFailCount+inSucessCount) eq 0?1:(inFailCount+inSucessCount))}" pattern="0.00%" /> 
				</td>
