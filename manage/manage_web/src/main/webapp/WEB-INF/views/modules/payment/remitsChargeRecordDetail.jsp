...
<body>
	...
	<ul class="nav nav-tabs">
		<li><a href="${ctx}/payment/remitsChargeRecord/">充值管理列表
		</a></li>
		<li class="active"><a href="javascript:void(0)">汇款充值明细</a></li>
	</ul><br/>
...
<%-- 		<div class="control-group">
	  ...
	  <%--   <div class="control-group">
		...
		<div class="form-actions">
			...
			<%-- <input id="btnPass" class="btn" type="button" value="审核通过" />
			<input id="btnRefuse" class="btn" type="button" value="审核拒绝" />
			<c:if test="${not empty exception and chargeRecord.payType != 'REMITS'}">
	            ...
	            <c:if test="${chargeRecord.status == 'CAGING'}">
			        <input id="processSuccess" class="btn btn-primary" type="button" value="处理为成功" onclick="processException('${chargeRecord.id}','CHARGE','1000')"/>
			        <input id="processFail" class="btn btn-warning" type="button"  value="处理为失败" onclick="processException('${chargeRecord.id}','CHARGE','1002')"/>
        		</c:if>
        	</c:if> --%>
		</div>
	</form:form>
	<input type="hidden" id="chargeId" value="${chargeRecord.chargeId}"/>
