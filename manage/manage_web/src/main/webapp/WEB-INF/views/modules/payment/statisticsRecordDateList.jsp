...
<head>
	...
	<script type="text/javascript">
		$(document).ready(function() {
            ...
            $(":radio[name='countType']").each(function(){
                if("${countType}"==$(this).val()){
...
/* 			$(":radio[name='groupTime']").change(function(){
			...
			//excel导出功能
		 	$("#exportExcel").on("click",function(){
				...
				var host = "${ctx}/payment/statisticsRecordDate/exportExcel";
				var url = host+"?merchantId="+merchantId+"&merchantCompany="+merchantCompany+"&allowSystem="+allowSystem+"&transType="+transType+"&productCode="+productCode+
						...
						"&beginStatisticsTime="+beginStatisticsTime+"&endStatisticsTime="+endStatisticsTime+"&groupType="+groupType+"&countType="+countType;
...
<body>
	<ul class="nav nav-tabs">
		<li class="active"><a href="${ctx}/payment/statisticsRecordDate/">财务统计报表列表（时间段）</a></li>
	</ul>
	<form:form id="searchForm" modelAttribute="statisticsRecord" action="${ctx}/payment/statisticsRecordDate/" method="post" class="breadcrumb form-search">
		<input id="pageNo" name="pageNo" type="hidden" value="${page.pageNo}"/>
		<input id="pageSize" name="pageSize" type="hidden" value="${page.pageSize}"/>
		<ul class="ul-form" style="min-width:1200px">
			<li style="width:80%">
				<label>统计方式：</label>
				<label><input name="countType" type="radio" value="1">按商户</label>
				<label><input name="countType" type="radio" value="2">按支付类型</label>
				<label><input name="countType" type="radio" value="3">按产品类型</label>
			</li>
			<li style="width:500px;"><label>成功时间：</label>
				...
				<form:select id="statisticsDate" path="statisticsDate" name="statisticsDate" class="input-mini" style="display: none">
			...
			<li style="display: none"><label>交易类型：</label>
			...
			<li style="display: none"><label>通道合作方：</label>
			...
			<li style="display: none"><label>银行名称：</label>
			...
			<li style="display: none"><label>银行卡类型：</label>
			...
			<li style="display: none"><label>升降排序：</label>
			...
			<li style="display: none"><label>排序类型：</label>
			...
			<li>
				...
				<%-- <input id="groupTimeValue" type="hidden" value="${groupTime}" > --%>
			</li>
			<li style="width:80%">
				<label>时段类型：</label>
				...
				<label><input name="groupType"  type="radio" value="day">按日</label>
				<label><input name="groupType" type="radio" value="month">按月</label>
				<label style="width:145px;"><input name="groupType" type="radio" value="quarter">按季</label>
		...
		<thead>
			<tr>
                <c:if test="${countType==1}">
                    ...
                    <th style="text-align: center">商户账号</th>
                    <th style="text-align: center">商户公司名称</th>
                    <c:forEach items="${head}" var="item" varStatus="status">
                        <c:if test="${item!=null && item!='' && status.index>2}">
                ...
                <c:if test="${countType==2}">
                    ...
                    <c:forEach items="${head}" var="item">
                        <c:if test="${item!=null && item!=''}">
                            <th style="text-align: center">${fns:getDictLabel(item, 'payType', '')}</th>
                        </c:if>
                    </c:forEach>
                </c:if>
				<c:if test="${countType==3}">
					...
					<c:forEach items="${head}" var="item">
						<c:if test="${item!=null && item!=''}">
							<th style="text-align: center">${fns:getDictLabel(item, 'ProductCodeType', '')}</th>
						</c:if>
					</c:forEach>
				</c:if>
                <c:if test="${countType==null||countType==''}">
                    <th style="text-align: center">${firstTitle}</th>
                    <c:forEach items="${head}" var="item">
                        <c:if test="${item!=null && item!=''}">
		...
		<tbody>
            <c:if test="${countType==1}">
                <c:forEach items="${finaceLists}" var="lists">
                    <tr>
                        <c:forEach items="${head}" var="headList" varStatus="status">
                            <c:if test="${headList!=null&&status.index<3}">
                                <td style="text-align: right">
                                    ${lists[headList]}
                                </td>
                            </c:if>
                            <c:if test="${headList!=null&&status.index>2}">
                                <td style="text-align: right">
            ...
            <c:if test="${countType>=2||countType==null||countType==''}">
                <c:forEach items="${finaceLists}" var="lists">
                    <tr>
