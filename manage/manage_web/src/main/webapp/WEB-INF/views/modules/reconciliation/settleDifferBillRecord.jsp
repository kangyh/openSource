...
<body>
	<ul class="nav nav-tabs">
		<li class="active"><a href="${ctx}/reconciliation/settleDifferBillRecord">异常明细列表</a></li>
	</ul>
	<form action="${ctx}/reconciliation/settleDifferBillRecord" method="post" id="formBtn"></form>
	<form:form id="searchForm" modelAttribute="settleDifferBillRecord" action="${ctx}/reconciliation/settleDifferBillRecord" method="post" class="breadcrumb form-search">
		...
		<ul class="ul-form">
			...
			<li><label>摘要：</label>
					<form:input path="remark" id="remark" value="${settleDifferRrecord.remark}" htmlEscape="false" maxlength="80" class="input-medium" placeholder="请输入摘要" style="width:210px;"/>
			</li>
		</ul>
		<ul class="ul-form">
			<li><label>差异类型：</label>
				<form:select id="differType" path="differType" style="width:225px;" class="input-xlarge">
					<form:option value="" label="-选择差异类型-"/>
					<c:forEach items="${differType}" var="differType">
						<form:option value="${differType.value}" label="${differType.name}"/>
					</c:forEach>
				</form:select>
			</li>
			...
			<li><label>入库时间：</label>
					...
					value="<fmt:formatDate value="${settleDifferBillRecord.beginOperEndTime}" pattern="yyyy-MM-dd"/>"
				...
				<input id="endOperEndTime" name="endOperEndTime" type="text" readonly="readonly" maxlength="20" style="width:95px;cursor: pointer" class="input-medium Wdate"
					value="<fmt:formatDate value="${settleDifferBillRecord.endOperEndTime}" pattern="yyyy-MM-dd"/>"
					onclick="WdatePicker({dateFmt:'yyyy-MM-dd',isShowClear:false});"/>
			</li>
	...
	<table id="contentTable" class="table table-striped table-bordered table-condensed">
		...
		<c:forEach items="${page.list}" var="settleDifferRrecord">
			<tr>
				...
				<td <c:choose>
					<c:when test="${settleDifferRrecord.differType=='重复入库'}">
						style="background:red"
					</c:when>
					<c:when test="${settleDifferRrecord.differType=='冲正单'}">
						style="background:#707c9b"
					</c:when>
					<c:when test="${settleDifferRrecord.differType=='差异表已存在'}">
						style="background:brown"
					</c:when>
				</c:choose>>
						${settleDifferRrecord.differType}
				</td>
