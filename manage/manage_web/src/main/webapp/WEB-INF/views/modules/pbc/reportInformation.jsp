...
<body>
	<ul class="nav nav-tabs">
		<li class="active"><a href="${ctx}/pbc/reportInformation">交易类信息列表</a></li>
	</ul>
	<form action="${ctx}/pbc/reportInformation/" method="post" id="formBtn"></form> 
	<form:form id="searchForm" modelAttribute="pbcFeedBack" action="${ctx}/pbc/reportInformation" method="post" class="breadcrumb form-search">
	<input id="pageNo" name="pageNo" type="hidden" value="${page.pageNo}"/>
	<input id="pageSize" name="pageSize" type="hidden" value="${page.pageSize}"/>
	<input id="showStatu" name="showStatu" type="hidden" value="${showStatu}"/>
		<ul class="ul-form">
			<li><label>申请时间：</label>
			...
			<li><label>业务申请编号：</label>
				<form:input path="applicationId"  htmlEscape="false" style="width:256px;" class="input-medium required"/>
			</li>
		</ul> 
		
	 	<ul class="ul-form">
	 		<li><label>传输报文流水：</label>
				<form:input path="transSerialNumber"  htmlEscape="false" style="width:256px;" class="input-medium required"/>
			</li>
			<li><label>上报状态：</label>
	...
	<table id="contentTable" class="table table-striped table-bordered table-condensed">
		...
		<c:forEach items="${page.list}" var="pbcFeedBack" varStatus="i">
				<tr>
				    ...
				    <td> 
						<c:choose>
				    		<c:when test="${not empty showStatu}">
				    			<c:if test="${pbcFeedBack.queryXml == 'ACCOUNTRANSDETAIL'}"><a href="${ctx}/pbc/pbcFeedBackQueryDealQuery/deal?feedBackId=${pbcFeedBack.feedBackId}&param=hidden">查看</a>
									</c:if>
								<c:if test="${pbcFeedBack.queryXml == 'ACCOUNTSUBJECTDETAIL'}"><a href="${ctx}/pbc/pbcFeedBackQueryDealQuery/deal?feedBackId=${pbcFeedBack.feedBackId}&param=hidden">查看</a>
									</c:if>
								<c:if test="${pbcFeedBack.queryXml == 'ACCOUNTDYNAMIC'}"><a href="${ctx}/pbc/pbcFeedBackQueryDealQuery/deal?feedBackId=${pbcFeedBack.feedBackId}&param=hidden">查看</a>
									</c:if>
								<c:if test="${pbcFeedBack.queryXml == 'ACCOUNTDYNAMICREMOVE'}"><a href="${ctx}/pbc/pbcFeedBackQueryDealQuery/deal?feedBackId=${pbcFeedBack.feedBackId}&param=hidden">查看</a>
									</c:if>
								<c:if test="${pbcFeedBack.queryXml == 'PAYMENYACCOUNT'}"><a href="${ctx}/pbc/pbcFeedBackQueryDealQuery/deal?feedBackId=${pbcFeedBack.feedBackId}&param=hidden">查看</a>
									</c:if>
								<c:if test="${pbcFeedBack.queryXml == 'TRANSNOACCOUNT'}"><a href="${ctx}/pbc/pbcFeedBackQueryDealQuery/deal?feedBackId=${pbcFeedBack.feedBackId}&param=hidden">查看</a>
									</c:if>
				    		</c:when>
				    		<c:otherwise>
				    				<c:if test="${pbcFeedBack.queryXml == 'ACCOUNTRANSDETAIL'}"><a href="${ctx}/pbc/pbcFeedBackQueryDealQuery/deal?feedBackId=${pbcFeedBack.feedBackId}&param=hidden">查看</a>
						                | <a  href="${ctx}/pbc/pbcFeedBackQueryDealQuery/deal?feedBackId=${pbcFeedBack.feedBackId}">审核</a></c:if>
									<c:if test="${pbcFeedBack.queryXml == 'ACCOUNTSUBJECTDETAIL'}"><a href="${ctx}/pbc/pbcFeedBackQueryDealQuery/deal?feedBackId=${pbcFeedBack.feedBackId}&param=hidden">查看</a>
										| <a  href="${ctx}/pbc/pbcFeedBackQueryDealQuery/deal?feedBackId=${pbcFeedBack.feedBackId}">审核</a></c:if>
									<c:if test="${pbcFeedBack.queryXml == 'ACCOUNTDYNAMIC'}"><a href="${ctx}/pbc/pbcFeedBackQueryDealQuery/deal?feedBackId=${pbcFeedBack.feedBackId}&param=hidden">查看</a>
										| <a  href="${ctx}/pbc/pbcFeedBackQueryDealQuery/deal?feedBackId=${pbcFeedBack.feedBackId}">审核</a></c:if>
