	...
	<script type="text/javascript">
		$(document).ready(function() {
			...
			$(".toAuthentication").on("click",function () {
				$(".loading_box").show();
				$.post("${ctx}/merchant/corporateIdentities/phoneAuth",{"merchantId":$("#userId").val()},function(data){
					$(".loading_box").hide();
					$("#authInfo").html(data);
...
<body>
	...
	<ul class="nav nav-tabs">
		...
		<li class="active"><a href="${ctx}/merchant/merchantRc/form?id=${merchant.userId}">风控审核</a></li>
	</ul><br/>
	<form:form id="inputForm" modelAttribute="merchant" action="${ctx}/merchant/merchantRc/status" method="post" class="form-horizontal" >
		...
		<div class="control-group">
			...
			<div class="control-right">
				<label class="control-label">联系人手机号认证 ：</label>
				<div class="controls">
					<p id="authInfo">${merchant.phoneAuthStatus}
						<c:if test="${empty merchant.phoneAuthStatus}">未认证</c:if>
						<c:if test="${merchant.phoneAuthStatus == '认证失败'}">,${merchant.phoneAuthReason}</c:if></p>
					<p class="go_renz">
						<c:if test="${merchant.phoneAuthStatus != '认证成功'}"><a style="cursor:pointer;" class="toAuthentication">去认证</a></c:if></p>
				</div>
			</div>
		</div>
