	...
	<script type="text/javascript">
		$(document).ready(function() {
			$(".checkPass").on("click",function(){
				...
				$.ajax({
					type : "POST",
					url : "${ctx}/adjust/adjustTradeAccount/isAdminUser",
					dataType : "json",
					async : false,
...
<body>
	<ul class="nav nav-tabs">
		<li class="active"><a href="${ctx}/adjust/adjustTradeAccount/toReviewList">调账审核列表</a></li>
	</ul>
	<form:form id="searchForm" modelAttribute="adjustAccount" action="${ctx}/adjust/adjustTradeAccount/toReviewList" method="post" class="breadcrumb form-search">
	...
	<table id="contentTable" class="table table-striped table-bordered table-condensed">
		...
		<c:forEach items="${page.list}" var="adjustAccount">
			<tr>
				<td><a href="${ctx}/adjust/adjustTradeAccount/reviewDetails?adjustId=${adjustAccount.adjustId}">
				...
				<td>
					...
					<c:if test="${adjustAccount.status == 'REVOKE'}">
						<font style="color:#CD2626;">已撤销</font>
					</c:if>
					<c:if test="${adjustAccount.status == 'ADOPT'}">
						<font style="color:#008800;">复核通过</font>
					</c:if>
				</td>
				<td>
				...
				<shiro:hasPermission name="adjust:adjustAccount:review">
					<td>
						<c:if test="${adjustAccount.status == 'AUDING'}">
		    				<a href="${ctx}/adjust/adjustTradeAccount/reviewDetails?adjustId=${adjustAccount.adjustId}">查看详情</a>&nbsp;&nbsp;
		    				<a style="cursor:pointer;" class="checkPass" value-url="${ctx}/adjust/adjustTradeAccount/toReviewPage?adjustId=${adjustAccount.adjustId}&auditStatus=true">复核</a>
						</c:if>
						<c:if test="${adjustAccount.status == 'REVOKE'}">
		    				<a href="${ctx}/adjust/adjustTradeAccount/reviewDetails?adjustId=${adjustAccount.adjustId}">查看详情</a>
						</c:if>
