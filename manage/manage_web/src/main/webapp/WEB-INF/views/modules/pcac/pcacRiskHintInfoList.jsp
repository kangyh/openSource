...
<body>
	<ul class="nav nav-tabs">
		<li class="active"><a href="${ctx}/pcac/pcacRiskHintInfo/">风险提示信息列表</a></li>
	</ul>
	<form action="${ctx}/pcac/pcacRiskHintInfo/" method="post" id="formBtn"></form>
	<form:form id="searchForm" modelAttribute="pcacRiskHintInfo" action="${ctx}/pcac/pcacRiskHintInfo/" method="post" class="breadcrumb form-search">
	...
	<ul class="ul-form">
		...
		<li><label>推送日期：</label>
				...
				value="<fmt:formatDate value="${pcacRiskHintInfo.beginCheckTime}" pattern="yyyy-MM-dd HH:mm:ss"/>"
				onclick="WdatePicker({dateFmt:'yyyy-MM-dd HH:mm:ss',isShowClear:false});"/> - 
			<input id="endCheckTime" name="endCheckTime" type="text" readonly="readonly"  class="input-medium Wdate"
				value="<fmt:formatDate value="${pcacRiskHintInfo.endCheckTime}" pattern="yyyy-MM-dd HH:mm:ss"/>"
				onclick="WdatePicker({dateFmt:'yyyy-MM-dd HH:mm:ss',isShowClear:false});"/>
		</li>
...
<!-- 		<li class="btns"><input id="btnExport" class="btn btn-primary" type="button" onclick="window.history.go(-1);" value="返回"/></li>
 -->		<div id="messageBox" style="font-size: 15px; color: red;"></div>
	...
	<table id="contentTable" class="table table-striped table-bordered table-condensed">
		...
		<tbody>
		<c:forEach items="${page.list}" var="pcacRiskHintInfo" varStatus="i">
			<tr>
				<td>${i.count}</td>
				<td>${pcacRiskHintInfo.regName}</td>
				<td>${pcacRiskHintInfo.cusName}</td>
				<td>${pcacRiskHintInfo.docType}</td>
				<td><fmt:formatDate value="${pcacRiskHintInfo.createTime}" type="both" pattern="yyyy-MM-dd HH:mm:ss"/></td>
				<td>${pcacRiskHintInfo.docCode}</td>
				<td>${pcacRiskHintInfo.docName}</td>
				<td>${pcacRiskHintInfo.legDocType}</td>
				<td>${pcacRiskHintInfo.legDocCode}</td>
				<td>${pcacRiskHintInfo.level}</td>
				<td>${pcacRiskHintInfo.riskType}</td>
				<td><fmt:formatDate value="${pcacRiskHintInfo.validDate}" type="both" pattern="yyyy-MM-dd HH:mm:ss"/></td>
				<td>${pcacRiskHintInfo.validStatus}</td>
			</tr>
		</c:forEach>
		</tbody>
	</table>
