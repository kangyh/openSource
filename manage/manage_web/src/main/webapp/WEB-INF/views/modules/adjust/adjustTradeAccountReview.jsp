...
<head>
	...
	<script type="text/javascript">
		...
		function confirmReview(flag){
			...
			$("#auditRemarkHidden").val(auditRemark);
			if(flag == '1'){
...
<body>
	<ul class="nav nav-tabs">
		<li><a href="${ctx}/adjust/adjustTradeAccount/toReviewList">调账审核列表</a></li>
	</ul><br/>
	<form:form id="reviewForm" modelAttribute="adjustAccount" action="${ctx}/adjust/adjustTradeAccount/review" method="post" class="form-horizontal">
		<input type="hidden" id="adjustId" name="adjustId" value="${adjustAccount.adjustId }"/>
		<input type="hidden" id="flag" name="flag"/>
		<input type="hidden" id="auditRemarkHidden" name="auditRemarkHidden"/>
	...
	<table id="contentTable" class="table table-striped table-bordered table-condensed">
		...
		<tbody id="tbody">
			<c:forEach items="${list}" var="adjustAccountDetail">
				<tr <c:if test="${adjustAccountDetail.explain =='手动补账' }">style="color:#008800"</c:if>>
					...
					<td>${adjustAccountDetail.explain }</td>
				</tr>
			</c:forEach>
		</tbody>
		<tfoot>
			<tr height="100px;">
				<th colspan="8">
					<li><label>审核备注：</label>
					<textarea id="auditRemark" name="auditRemark" rows="3" htmlEscape="false" class="input-xlarge required" style="width:400px"></textarea>
					<font color="red">*</font>
					</li>
				</th>
			</tr>
	...
	<div class="form-actions">
	   <input type="button" class="btn" value="返回" onclick="javascript:window.location='${ctx}/adjust/adjustTradeAccount/toReviewList';"/>&nbsp;&nbsp;&nbsp;					
