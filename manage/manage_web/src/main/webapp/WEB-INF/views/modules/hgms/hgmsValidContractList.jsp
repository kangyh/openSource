...
<head>
	...
	<script type="text/javascript">
        ...
        function onClear(){
            ...
            $("#contractStatus").find("option:eq(0)").attr("selected", "selected");
            $(".select2-chosen:eq(0)").text($("#contractStatus option[selected]").text());
        }
	</script>
</head>
<body>
	<ul class="nav nav-tabs">
		<li class="active"><a href="${ctx}/hgms/hgmsValidContract/">有效合约列表</a></li>
	</ul>
	<form:form id="searchForm" modelAttribute="hgmsValidContract" action="${ctx}/hgms/hgmsValidContract/" method="post" class="breadcrumb form-search">
	...
	<table id="contentTable" class="table table-striped table-bordered table-condensed">
		...
		<c:forEach items="${page.list}" var="hgmsValidContract">
			<tr>
				...
				<td>
					${hgmsValidContract.status}
				</td>
                <td>
                    <fmt:formatDate value="${hgmsValidContract.enteringTime}" pattern="yyyy-MM-dd HH:mm:ss"/>
                </td>
				<shiro:hasPermission name="hgms:hgmsValidContract:edit"><td>
    				<a href="${ctx}/hgms/hgmsInvalidContract/?merchantId=${hgmsValidContract.merchantId}">失效合约</a>
					<a href="${ctx}/hgms/hgmsValidContract/form?id=${hgmsValidContract.contractId}">详情</a>
                    <c:if test="${hgmsValidContract.contractStatus == '正常'}">
                        <a href="${ctx}/hgms/hgmsValidContract/status?contractId=${hgmsValidContract.contractId}&contractStatus=FREZED">禁用</a>
                    </c:if>
