	...
	<script type="text/javascript">
			$(document).ready(function() {
				...
				$(".add_list").on("click",function(){
					...
					$.ajax({
						type: "GET",
						url: "${ctx}/route/productChannel/addChannel",
						data: {'productId':productId,'id':payChannelId,'productCode':productCode,'productName':productName,'bankNo':bankNo,'channelPartnerCode':channelPartnerCode,'channelTypeCode':channelTypeCode,'cardTypeCode':cardTypeCode,'pageNo':pageNo},
								success: function(){
				...
				$("#add_channel").on("click",function(){
					...
					$.ajax({
						type: "GET",
						url: "${ctx}/route/productChannel/addBatchChannel",
						data: {'productId':productId,'checkedstr':checkedstr,'productCode':productCode,'productName':productName,'bankNo':bankNo,'channelPartnerCode':channelPartnerCode,'channelTypeCode':channelTypeCode,'cardTypeCode':cardTypeCode,'pageNo':pageNo},
						success: function(){
							window.location.reload();
						},
						error: function(){
...
<body>
	<form:form id="searchForm" modelAttribute="payChannel" action="${ctx}/route/productChannel/add" method="post" class="breadcrumb form-search">
	...
	<table id="contentTable" class="table table-striped table-bordered table-condensed" style="width:1200px">
		<thead>
			<tr>
				...
				<th>支付通道类型</th>
				<th>银行卡类型</th>
				<shiro:hasPermission name="route:productChannel:edit"><th style="width:150px">操作</th></shiro:hasPermission>
			</tr>
		</thead>
		<tbody class="record_list">
		<c:forEach items="${page.list}" var="payChannel">
			<tr>
				<c:if test="${payChannel.productDetailId!=null}"><td><input type="checkbox" checked="checked" disabled="disabled"></td></c:if>
				<c:if test="${empty payChannel.productDetailId}"><td><input type="checkbox" class="checkbox" value="${payChannel.id}" name="app_box"></td></c:if>
				...
				<c:if test="${payChannel.productDetailId!=null}"><span>已增加</span></c:if>
				<c:if test="${empty payChannel.productDetailId}">
	...
	<div class="form-actions">
		...
		<input type="button" class="btn" value="返回" onclick="javascript:window.location='${ctx}/route/productChannel/details?id=${productId}';"/>
	</div>
