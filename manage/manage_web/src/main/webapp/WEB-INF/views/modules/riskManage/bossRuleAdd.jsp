...
<head>
	...
	$(document).ready(function() {
	 ...
	 function CheckStatus(){
			...
			$.ajax({
	            type: "GET",
	            url: "${ctx}/risk/bossRule/checkStatus",
	            cache:false,
	            data:{
	            	"status":status
				},
	            success: function(data){
	                if(1!=data){
	                	$("#jobStatusId").text("存在未执行的数据,不能再次添加未执行的数据");
	                	$("#remark").val("");
	                }else{
	                	$("#jobStatusId").text("");
	                }
...
<body>
	...
	<form:form id="inputForm" modelAttribute="bossRule" action="${ctx}/risk/bossRule/save" method="post" class="form-horizontal">
		...
		<div class="control-group">
			...
			<div class="controls">
				<form:select id="jobStatus" path="jobStatus"   class="input-xlarge required" >
					<c:forEach items="${jobStatus}" var="jobStatuss">
						<form:option value="${jobStatuss.value}" label="${jobStatuss.name}"/>
					</c:forEach>
				</form:select>
				<span id="jobStatusId" style="color:red;" class="help-inline"><font color="red">*</font> </span>
			</div>
		</div>
		...
		<div class="control-group">
			...
			<div class="controls">
				<form:input id="remark" path="remark"  onchange="CheckStatus()" htmlEscape="false"  maxlength="30" class="input-xlarge "/>
				<span id="remarkId" style="color:red;" class="help-inline"><font color="red">*</font> </span>
			</div>
		</div>
