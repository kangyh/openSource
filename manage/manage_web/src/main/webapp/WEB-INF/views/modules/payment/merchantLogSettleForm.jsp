...
<body>
	<ul class="nav nav-tabs">
		<li><a href="${ctx}/payment/merchantLogSettle/">结算明细查询列表</a></li>
		<li class="active"><a href="javascript:void(0)">结算明细详情查看</a></li>
	</ul><br/>
	<form:form id="inputForm" modelAttribute="merchantLog" action="${ctx}/payment/merchantLogSettle/save" method="post" class="form-horizontal">
		<form:hidden path="id"/>
		...
		<div class="control-group">
			...
			<div class="controls">
				<c:choose>
					<c:when test="${not empty merchantLog.transNo}">
					${merchantLog.transNo }
					</c:when>
					<c:otherwise>-</c:otherwise>
				</c:choose>
		...
		<%--<div class="control-group">--%>
			...
			<%--<div class="controls">--%>
			<%--<fmt:formatDate value="${merchantLog.updateTime }" type="both" pattern="yyyy-MM-dd HH:mm:ss"/>--%>
				<%----%>
			<%--</div>--%>
		<%--</div>--%>
		<%--<div class="control-group">--%>
			<%--<label class="control-label">流水描述：</label>--%>
			<%--<div class="controls">--%>
				<%--${merchantLog.description }--%>
			<%--</div>--%>
		<%--</div>--%>
		<%--<div class="control-group">--%>
			<%--<label class="control-label">流水备注：</label>--%>
			<%--<div class="controls">--%>
				<%--${merchantLog.remark }--%>
			<%--</div>--%>
		<%--</div>--%>
		...
		<div class="form-actions">
			<shiro:hasPermission name="payment:merchantLog:edit"><input id="btnSubmit" class="btn btn-primary" type="submit" value="保 存"/>&nbsp;</shiro:hasPermission>
