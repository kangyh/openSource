...
<body>
	<ul class="nav nav-tabs">
		<li class="active"><a href="${ctx}/settle/settleMerchantRecordHis/">商户结算历史列表</a></li>
	</ul>
	<form:form id="searchForm" modelAttribute="settleMerchantRecordHis" action="${ctx}/settle/settleMerchantRecordHis/" method="post" class="breadcrumb form-search">
	...
	<table id="contentTable" class="table table-striped table-bordered table-condensed">
		...
		<tbody>
		<c:forEach items="${page.list}" var="settleMerchantRecordHis">
			<tr>
				<td>${settleMerchantRecordHis.merchantId}</td>
				<td>${settleMerchantRecordHis.transType}</td>
				<td>${settleMerchantRecordHis.currency}</td>
				<td>${settleMerchantRecordHis.payNum}</td>
				<td>
					<fmt:formatNumber value="${settleMerchantRecordHis.totalAmount}" pattern="￥0.0000" />
				</td>
				<td><fmt:formatDate value="${settleMerchantRecordHis.checkTime}" type="both" pattern="yyyy-MM-dd"/></td>
				<td><fmt:formatDate value="${settleMerchantRecordHis.settleTime}" type="both" pattern="yyyy-MM-dd"/></td>
				<td>${settleMerchantRecordHis.settleBath}</td>
				<td>
					<fmt:formatNumber value="${settleMerchantRecordHis.settleAmount}" pattern="￥0.0000" />
				</td>
				<td>${settleMerchantRecordHis.settleCyc}</td>
				<td>
					<fmt:formatNumber value="${settleMerchantRecordHis.totalFee}" pattern="￥0.0000" />
				</td>
				<td <c:choose>
			     		<c:when test="${settleMerchantRecordHis.settleStatus=='已结算'}">
			     		 style="background:#6cf683" 	
			     		</c:when>
			     		<c:when test="${settleMerchantRecordHis.settleStatus=='未结算'}">
			     		 style="background:#707c9b" 
			     		</c:when>
			     		<c:when test="${settleMerchantRecordHis.settleStatus=='结算中'}">
			     		 style="background:#ffb300" 
			     		</c:when>
			     	</c:choose>>${settleMerchantRecordHis.settleStatus}</td>
				<td>
					<a href="${ctx}/settle/settleMerchantRecordHis/toDetail?settleBath=${settleMerchantRecordHis.settleBath}">查看明细</a>
				</td>
			</tr>
