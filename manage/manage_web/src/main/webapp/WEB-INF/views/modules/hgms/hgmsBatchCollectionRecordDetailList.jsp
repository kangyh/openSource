...
<body>
	<ul class="nav nav-tabs">
		<li class="active"><a href="${ctx}/hgms/hgmsBatchCollectionRecordDetail/">当日收款记录明细查询列表</a></li>
	</ul>
	<form:form id="searchForm" modelAttribute="hgmsBatchCollectionRecordDetail" action="${ctx}/hgms/hgmsBatchCollectionRecordDetail/" method="post" class="breadcrumb form-search">
		...
		<ul class="ul-form">
			...
			<li><label>创建时间：</label>
				<input name="beginCreateTime" type="text" readonly="readonly" maxlength="20" class="input-medium Wdate"
					   value="<fmt:formatDate value="${hgmsBatchCollectionRecordDetail.beginCreateTime}" pattern="yyyy-MM-dd HH:mm:ss"/>"
					   onclick="WdatePicker({dateFmt:'yyyy-MM-dd HH:mm:ss',isShowClear:false});"/> -
				<input name="endCreateTime" type="text" readonly="readonly" maxlength="20" class="input-medium Wdate"
					   value="<fmt:formatDate value="${hgmsBatchCollectionRecordDetail.endCreateTime}" pattern="yyyy-MM-dd HH:mm:ss"/>"
					   onclick="WdatePicker({dateFmt:'yyyy-MM-dd HH:mm:ss',isShowClear:false});"/>
			</li>
			<li><label>交易批次号：</label>
			<input  name="transBatchNo" type="text" value="${hgmsBatchCollectionRecordDetail.transBatchNo}"/>
			</li>
			<li><label>交易方式：</label>
			...
			<li><label style="padding-left: 15px">状态：</label>
	...
	<table id="contentTable" class="table table-striped table-bordered table-condensed">
		...
		<tbody>
		<c:forEach items="${page.list}" var="hgmsBatchCollectionRecordDetail">
			<tr>
				<td>
						${hgmsBatchCollectionRecordDetail.transBatchNo}
				</td>
				<td>
					${hgmsBatchCollectionRecordDetail.transId}
				</td>
				<td>
						${hgmsBatchCollectionRecordDetail.hfbTransId}
				</td>
				<td>
					${hgmsBatchCollectionRecordDetail.sourceMerchantName}
				</td>
				<td>
					${hgmsBatchCollectionRecordDetail.destMerchantName}
				</td>
				<td>
						${hgmsBatchCollectionRecordDetail.openbankName}
				</td>
				<td>
					<fmt:formatDate value="${hgmsBatchCollectionRecordDetail.createTime}" pattern="yyyy-MM-dd HH:mm:ss"/>
				</td>
				<td>
						${hgmsBatchCollectionRecordDetail.transWay}
				</td>
				<td>
					${hgmsBatchCollectionRecordDetail.transStatus}
				</td>
				<td>
						${hgmsBatchCollectionRecordDetail.settlementStatus}
				</td>
				<td>
						<fmt:formatNumber value="${hgmsBatchCollectionRecordDetail.feeAmount}" pattern="￥0.00" />
				</td>
				<td>
					<fmt:formatNumber value="${hgmsBatchCollectionRecordDetail.collectAmount}" pattern="￥0.00" />
				</td>
				<td>
					${hgmsBatchCollectionRecordDetail.errorMsg}
				</td>
			</tr>
		</c:forEach>
		</tbody>
