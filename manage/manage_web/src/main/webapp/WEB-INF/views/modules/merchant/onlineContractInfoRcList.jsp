...
<body>
	<ul class="nav nav-tabs">
		<li class="active"><a href="${ctx}/merchant/onlineContractInfoRc/">产品签约列表</a></li>
	</ul>
	<form:form id="searchForm" modelAttribute="onlineContractInfo" action="${ctx}/merchant/onlineContractInfoRc/" method="post" class="breadcrumb form-search">
		...
		<ul class="ul-form">
			...
			<li><label>审核状态：</label>
				<form:select id="rcAuditStatus" path="rcAuditStatus" class="input-xlarge">
	...
	<table id="contentTable" class="table table-striped table-bordered table-condensed">
		<thead>
			<tr>
				...
				<th>创建时间</th>
				<th>状态</th>
				<shiro:hasPermission name="merchant:onlineContractInfoRc:edit"><th>操作</th></shiro:hasPermission>
			</tr>
		</thead>
		<tbody>
		<c:forEach items="${page.list}" var="onlineContractInfo">
			<tr>
				...
				<td>${onlineContractInfo.rcAuditStatus}</td>
				<shiro:hasPermission name="merchant:onlineContractInfoRc:edit"><td>
					<c:if test="${onlineContractInfo.rcAuditStatus != '审核通过'}">
						<a style="cursor:pointer;" class="checkPass" value-url="${ctx}/merchant/onlineContractInfoRc/audit?batchNo=${onlineContractInfo.batchNo}&userId=${onlineContractInfo.userId}">审核</a>
					</c:if>
					<a href="${ctx}/merchant/onlineContractInfoRc/detail?batchNo=${onlineContractInfo.batchNo}&userId=${onlineContractInfo.userId}">详情</a>
