...
<body>
	<ul class="nav nav-tabs">
		<li class="active"><a href="${ctx}/hgms/hgmsBatchPayDetailHistory/">历史代付记录明细查询列表</a></li>
	</ul>
	<form:form id="searchForm" modelAttribute="hgmsBatchPayDetailHistory" action="${ctx}/hgms/hgmsBatchPayDetailHistory/" method="post" class="breadcrumb form-search">
		...
		<ul class="ul-form">
			...
			<li><label>交易时间：</label>
				<input name="beginCreateTime" type="text" readonly="readonly" maxlength="20" class="input-medium Wdate"
					value="<fmt:formatDate value="${hgmsBatchPayDetailHistory.beginCreateTime}" pattern="yyyy-MM-dd HH:mm:ss"/>"
					onclick="WdatePicker({dateFmt:'yyyy-MM-dd HH:mm:ss',isShowClear:false});"/> - 
				<input name="endCreateTime" type="text" readonly="readonly" maxlength="20" class="input-medium Wdate"
					value="<fmt:formatDate value="${hgmsBatchPayDetailHistory.endCreateTime}" pattern="yyyy-MM-dd HH:mm:ss"/>"
					onclick="WdatePicker({dateFmt:'yyyy-MM-dd HH:mm:ss',isShowClear:false});"/>
			</li>
			<li><label>订单批次号：</label>
			<input  name="transBatchNo" type="text" value="${hgmsBatchPayDetailHistory.transBatchNo}"/>
			</li>
			<li><label>交易方式：</label>
	...
	<table id="contentTable" class="table table-striped table-bordered table-condensed">
		...
		<tbody>
		<c:forEach items="${page.list}" var="hgmsBatchPayDetailHistory">
			<tr>
				<td>
						${hgmsBatchPayDetailHistory.transBatchNo}
				</td>
				<td>
					${hgmsBatchPayDetailHistory.transId}
				</td>
				<td>
						${hgmsBatchPayDetailHistory.hfbTransId}
				</td>
				<td>
					${hgmsBatchPayDetailHistory.sourceMerchantName}
				</td>
				<td>
					${hgmsBatchPayDetailHistory.destMerchantName}
				</td>
				<td>
						${hgmsBatchPayDetailHistory.openbankName}
				</td>

				<td>
					<fmt:formatDate value="${hgmsBatchPayDetailHistory.createTime}" pattern="yyyy-MM-dd HH:mm:ss"/>
				</td>
				<td>
					${hgmsBatchPayDetailHistory.transWay}
				</td>
				<td>
					${hgmsBatchPayDetailHistory.transStatus}
				</td>
				<td>
						${hgmsBatchPayDetailHistory.settlementStatus}
				</td>
				<td>
						<fmt:formatNumber value="${hgmsBatchPayDetailHistory.feeAmount}" pattern="￥0.00" />
				</td>
				<td>
					<fmt:formatNumber value="${hgmsBatchPayDetailHistory.payAmount}" pattern="￥0.00" />
				</td>
				<td>
					${hgmsBatchPayDetailHistory.errorMsg}
				</td>
			</tr>
		</c:forEach>
		</tbody>
