...
<body>
	<ul class="nav nav-tabs">
		<li class="active"><a href="${ctx}/cbms/merchant/merchantLegal/">法务审核列表</a></li>
	</ul>
	<form:form id="searchForm" modelAttribute="merchant" action="${ctx}/cbms/merchant/merchantLegal/" method="post" class="breadcrumb form-search">
	...
	<table id="contentTable" class="table table-striped table-bordered table-condensed">
		...
		<c:forEach items="${page.list}" var="merchant">
			<tr>
				...
				<td><c:out value="${merchant.type}"/></td>
				<td><fmt:formatDate value="${merchant.createTime}" pattern="yyyy-MM-dd HH:mm:ss"/></td>
				<td>${merchant.legalAuditStatus}</td>
				<shiro:hasPermission name="merchant:merchantLegal:edit">
					<td>
						<a href="${ctx}/cbms/merchant/merchantLegal/detail?id=${merchant.userId}">详情</a>
						<c:if test="${merchant.legalAuditStatus == '待审核'}">
							<a href="${ctx}/cbms/merchant/merchantLegal/form?id=${merchant.userId}">审核</a>
						</c:if>
						<c:if test="${merchant.legalAuditStatus == '审核不通过'}">
							<a href="${ctx}/cbms/cbmsMerchantSuppliersetting/edit?id=${merchant.userId}&goal=1">修改</a>
						</c:if>
					</td>
				</shiro:hasPermission>
			</tr>
