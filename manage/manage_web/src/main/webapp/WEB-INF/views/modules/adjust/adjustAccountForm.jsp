...
<head>
	...
	function addAdjustAccount(obj){
		...
		htmlDiv.push('<td><select name="type" style="width:90px;"><option value="1">个人账户</option><option value="2">商户账户</option><option value="3">内部账户</option><option value="4">存管账户</option><option value="5">资金归集账户</option><option value="6">用户账户</option></select></td>');
		...
		htmlDiv.push('<td name="remarkTd">--</td>');
	...
	function deleteAdjustAccount(obj){
		...
		$.each(trs, function(i, item){
			trs.find("input[name='seno']").eq(i).val(i+1);
		});
		...
		$.ajax({
			...
			success : function(data){
				if(data == null){
					...
					$("#tbody > tr").eq(index).find("td[name='remarkTd']").html(remark);
				}
			}
		});
	}
...
<body>
	<ul class="nav nav-tabs">
		...
		<li class="active"><a href="${ctx}/adjust/adjustAccount/form">调账申请</a></li>
	</ul><br/>
	<form id="inputForm" modelAttribute="adjustAccount" action="${ctx}/adjust/adjustAccount/save" method="post" class="form-horizontal">
		...
		<table id="contentTable" class="table table-striped table-bordered table-condensed">
			...
			<tbody id="tbody">
				<tr>
					<td><input name="seno" value="1" style="width:20px" readonly="true"/></td>
					<td>
						<select name="type" style="100px;">
		                    <option value="1">个人账户</option>
		                    <option value="2">商户账户</option>
		                    <option value="3">内部账户</option>
							<option value="4">存管账户</option>
							<option value="5">资金归集账户</option>
							<option value="6">用户账户</option>
						</select>
					</td>
					<td>
			...
			<tfoot>
				<tr height="50px;">
					<th colspan="9">
						总笔数：<label id="totalCount">0</label>&nbsp;&nbsp;&nbsp;
						借方总金额：<label id="totalDebitAmount">0</label>&nbsp;&nbsp;&nbsp;
						贷方总金额：<label id="totalCreditAmount">0</label>
					</th>
				</tr>
				<tr height="60px;" >
					<th colspan="9">
