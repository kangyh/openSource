...
<head>
	...
	<script type="text/javascript">
        ...
        function onClear(){
            ...
            $("#extend1").find("option:eq(0)").attr("selected", "selected");
            $(".select2-chosen:eq(0)").text($("#extend1 option[selected]").text());
        }
	</script>
</head>
<body>
	<ul class="nav nav-tabs">
		<li class="active"><a href="${ctx}/hgms/hgmsValidContractRcAudit/">合约风控列表</a></li>
	</ul>
	<form:form id="searchForm" modelAttribute="hgmsValidContract" action="${ctx}/hgms/hgmsValidContractRcAudit" method="post" class="breadcrumb form-search">
		...
		<ul class="ul-form">
			...
			<li><label>状态：</label>
				<form:select path="rcAuditStatus" class="input-medium">
	...
	<table id="contentTable" class="table table-striped table-bordered table-condensed">
		...
		<c:forEach items="${page.list}" var="hgmsValidContract">
			<tr>
				...
				<td>
						${hgmsValidContract.rcAuditStatus}
				</td>
				<td>
					<fmt:formatDate value="${hgmsValidContract.enteringTime}" pattern="yyyy-MM-dd HH:mm:ss"/>
				</td>
				<shiro:hasPermission name="hgms:hgmsValidContractRcAudit:edit"><td>
					<c:if test="${hgmsValidContract.rcAuditStatus == '待审核'}">
						<a href="${ctx}/hgms/hgmsValidContractRcAudit/form?id=${hgmsValidContract.contractId}">审核</a>
					</c:if>
					<c:if test="${hgmsValidContract.rcAuditStatus == '审核不通过'}">
						<a href="${ctx}/hgms/hgmsValidContractRcAudit/edit?id=${hgmsValidContract.contractId}">修改</a>
					</c:if>
					<c:if test="${hgmsValidContract.rcAuditStatus == '审核通过'}">
						<a href="${ctx}/hgms/hgmsValidContractRcAudit/detail?id=${hgmsValidContract.contractId}">查看</a>
					</c:if>
