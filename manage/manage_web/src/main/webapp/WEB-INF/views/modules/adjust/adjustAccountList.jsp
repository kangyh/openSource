...
<head>
	...
	<script type="text/javascript">
		$(document).ready(function() {
			$("#addAdjustAccount").on("click", function(){
				var url = $("#addAdjustAccount").attr("value-url");
				$.ajax({
					type : "POST",
...
<body>
	<ul class="nav nav-tabs">
		<li class="active"><a href="${ctx}/adjust/adjustAccount/">调账列表</a></li>
	</ul>
	<form:form id="searchForm" modelAttribute="adjustAccount" action="${ctx}/adjust/adjustAccount/" method="post" class="breadcrumb form-search">
		...
		<ul class="ul-form">
			...
			<li class="btns"><a id="addAdjustAccount" class="btn btn-warning" value-url="${ctx}/adjust/adjustAccount/form">调账申请</a></li>
	...
	<table id="contentTable" class="table table-striped table-bordered table-condensed">
		<thead>
			<tr>
				...
				<th>调账原因</th>
				<th>记账时间</th>
				<shiro:hasPermission name="adjust:adjustAccount:edit"><th>操作</th></shiro:hasPermission>
			</tr>
		</thead>
		<tbody>
		<c:forEach items="${page.list}" var="adjustAccount">
			<tr>
				<td><a href="${ctx}/adjust/adjustAccount/details?adjustId=${adjustAccount.adjustId}">
				...
				<td>
    				<a href="${ctx}/adjust/adjustAccount/details?adjustId=${adjustAccount.adjustId}">查看详情</a>&nbsp;&nbsp;
    				<a href="${ctx}/adjust/adjustAccount/toUploadPage?adjustId=${adjustAccount.adjustId}">上传图片</a>
				</td>
			</tr>
		</c:forEach>
		</tbody>
