...
<head>
	...
	<script type="text/javascript">
		...
		var validateFrom = {
			validate: function(form){
				...
				if(!checkSafe(cusCode)){
					//$("#messageBox").text("商户代码包含非法字符!");
					alertx("商户代码包含非法字符!");
					return ;
				}
				if(!checkSafe(regName)){
					//$("#messageBox").text("商户名称包含非法字符!");
					alertx("商户名称包含非法字符!");
					return ;
				}
				if(!checkSafe(uploader)){
					//$("#messageBox").text("申请人包含非法字符!");
					alertx("申请人包含非法字符!");
					return ;
				}
				if(!checkSafe(reviewer)){
					//$("#messageBox").text("审核人包含非法字符!");
					alertx("审核人包含非法字符!");
					return ;
				}
				if(!checkSafe(repPerson)){
					//$("#messageBox").text("上报人包含非法字符!");
					alertx("上报人包含非法字符!");
					return ;
				}
		...
		function onExport(){
	        ...
	        $("#searchForm").attr("action","${ctx}/pcac/pcacPersonReport/view/export");
			validateFrom.validate($("#searchForm"));
		...
		function checkDelete() {
			...
			if(x==0) {
				...
				loading('正在提交，请稍等...');
				$.ajax({
					url:"${ctx}/pcac/pcacPersonReport/view/deleteManyData",
					type:"post",
					cache:false,
					timeout:10000,
					data:{
		...
		function checkReport() {
			...
			if(x==0) {
				...
				loading('正在提交，请稍等...');
				$.ajax({
					url:"${ctx}/pcac/pcacPersonReport/view/reportManyData",
					type:"post",
					cache:false,
					timeout:10000,
					data:{
...
<body>
	<ul class="nav nav-tabs">
		<li class="active"><a href="${ctx}/pcac/pcacPersonReport/view/handle">个人商户信息上报处理</a></li>
	</ul>
	<form:form id="searchForm" modelAttribute="pcacPersonReport" action="${ctx}/pcac/pcacPersonReport/view/handle" method="post" class="breadcrumb form-search">
		...
		<tbody>
			<c:forEach items="${page.list}" var="ppReportInfo" varStatus="seq">
				<tr id="tr_${ppReportInfo.personReportId}">
					...
					<c:if test="${pcacPersonReport.opStatus eq '01' or pcacPersonReport.opStatus eq '02' or pcacPersonReport.opStatus eq '03'   }">
						<td><input type="checkbox" name="personReportId" value="${ppReportInfo.personReportId}" class="exportParam"/></td>
					...
					<td>
							...
							<%-- <shiro:hasPermission name="pcac:pcacPersonReport:edit">
									<c:if test="${pcacPersonReport.opStatus eq '01' }">
									<a href="${ctx}/pcac/pcacPersonReport/view/viewDetail?personReportId=${ppReportInfo.personReportId}&param=hidden">单笔审核</a>
									<a href="${ctx}/pcac/pcacPersonReport/view/deleteOneData?personReportId=${ppReportInfo.personReportId}&param=hidden">单笔删除</a>
									</c:if>
									<c:if test="${pcacPersonReport.opStatus eq '02' }">
										<a href="${ctx}/pcac/pcacPersonReport/view/reportOneData?personReportId=${ppReportInfo.personReportId}&param=hidden">单笔上报</a>
									</c:if>
									
							</shiro:hasPermission> --%>
							
							<c:if test="${pcacPersonReport.opStatus eq '01' or pcacPersonReport.opStatus eq '02'   or pcacPersonReport.opStatus eq '03'  }">
								<a href="${ctx}/pcac/pcacPersonReport/view/viewDetail?personReportId=${ppReportInfo.personReportId}&type=handle&param=hidden">详情</a>
							</c:if>
							<c:if test="${pcacPersonReport.opStatus eq '04' or pcacPersonReport.opStatus eq '05'   or pcacPersonReport.opStatus eq '06'}">
							<a href="${ctx}/pcac/pcacPersonReport/view/viewBatchDetail?batchNo=${ppReportInfo.batchNo}&opStatus=${pcacPersonReport.opStatus}&type=handle&pageNo=1">批次详情</a>
							</c:if>
					</td>			
				</tr>
			</c:forEach>
	...
	<div class="form-actions">
			<c:if test="${pcacPersonReport.opStatus ne '04' and pcacPersonReport.opStatus ne '05'  and pcacPersonReport.opStatus ne '06' }">
			<div class="control-left" style="margin-right: 10px;">
			<form id="deleteForm" action="${ctx}/pcac/pcacPersonReport/view/deleteManyData" method="post"  >
			...
			<form id="syndicInitForm" modelAttribute="pcacPersonReport" action="${ctx}/pcac/pcacPersonReport/view/initSyndicBatch" method="post"  >
			...
			<c:if test="${pcacPersonReport.opStatus eq '02'  }">
			<div class="control-right" style="margin-right: 10px;">
			<form id="reportForm" action="${ctx}/pcac/pcacPersonReport/view/reportManyData" method="post"  >
