...
<body>
	<ul class="nav nav-tabs">
		<li class="active"><a href="${ctx}/merchant/merchantRc/">商户风控列表</a></li>
	</ul>
	<form:form id="searchForm" modelAttribute="merchant" action="${ctx}/merchant/merchantRc/" method="post" class="breadcrumb form-search">
	...
	<table id="contentTable" class="table table-striped table-bordered table-condensed">
		...
		<c:forEach items="${page.list}" var="merchant">
			<tr>
				...
				<shiro:hasPermission name="merchant:merchantRc:edit"><td>
    				<a href="${ctx}/merchant/merchantRc/detail?id=${merchant.userId}">详情</a>
					<c:if test="${merchant.rcAuditStatus == '待审核'}">
					    <a style="cursor:pointer;" class="checkPass" value-url="${ctx}/merchant/merchantRc/form?id=${merchant.userId}">修改</a>
						<a style="cursor:pointer;" class="checkPass" value-url="${ctx}/merchant/merchantRc/audit?id=${merchant.userId}">审核</a>
	    			</c:if>
					<%-- <a href="${ctx}/merchant/merchantRc/delete?id=${merchant.userId}" onclick="return confirmx('确认要删除该merchant吗？', this.href)">删除</a> --%>
