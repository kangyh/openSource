...
<script type="text/javascript">
	...
	function loadSave(){
	    ...
	    $("#inputForm").attr("action","${ctx}/pbc/pbcPaymentAccountTwo/view/two/load?feedBackId="+feedBackId).submit();
	...
	function saveTwo(){
		...
		if(redioValue=="Y"){
			$("#inputForm").attr("action","${ctx}/pbc/pbcPaymentAccountTwo/view/two/save?redioValue=Y&feedBackId="+feedBackId).submit();
		}else{
			$("#inputForm").attr("action","${ctx}/pbc/pbcPaymentAccountTwo/view/two/save?feedBackId="+feedBackId).submit();
		}
	}
	...
	function page(n,s){
		...
		if(viewLook!=''){
			...
			$("#searchForm").attr("action","${ctx}/pbc/pbcPaymentAccountTwo/view/two?no=true&pageNo="+n);   
		}else{
			
			$("#searchForm").attr("action","${ctx}/pbc/pbcPaymentAccountTwo/view/two?pageNo="+n);
		} 
...
<body>
	...
	<form:form id="inputForm" modelAttribute="pbcPaymentAccount" action="${ctx}/pbc/pbcPaymentAccountTwo/view/two/save" method="post" class="form-horizontal">
			...
			<div class="control-right">
				<label class="control-label">上报失败原因：</label>
					...
					<div class="control-right">
						...
						<div class="controls">
						<form:input path="remark" id="remark" value="${pbcPaymentAccount.remark}" readOnly="true" style="border:0px;background-color:#fff;padding-top: 3px;"/>
						<span style="visibility: hidden;" class="help-inline"><font color="red">*</font> </span>
					</div>
			</div>
		</c:if> --%>
	...
	<table id="contentTable" class="table table-striped table-bordered table-condensed">
		...
		<c:forEach items="${page.list}" var="pbcPaymentAccount">
				<tr>
					<td>${pbcPaymentAccount.featureCode}</td>
					...
					<c:if test="${empty ngp}">
						<td>
							<a href="${ctx}/pbc/pbcPaymentAccount/view/add?local=two&paymentAccountId=${pbcPaymentAccount.accountNumber}&applicationId=${applicationIdLoad}&feedBackId=${feedBackId}">添加银行卡信息</a>|
							<a href="${ctx}/pbc/pbcPaymentAccountTwo/view/two/add?paymentAccountId=${pbcPaymentAccount.accountNumber}&applicationId=${applicationIdLoad}&feedBackId=${feedBackId}">添加交易信息</a>
						</td>
					</c:if>
				</tr>
	...
	<form id="loadForm" action="${ctx}/pbc/pbcPaymentAccountTwo/view/two/loadModel?applicationId=${pbcPaymentAccount.applicationId}&feedBackId=${feedBackId}" method="post" enctype="multipart/form-data" class="form-horizontal">
		<div class="control-group" style="margin-top:30px">
			<div class="control-left">
