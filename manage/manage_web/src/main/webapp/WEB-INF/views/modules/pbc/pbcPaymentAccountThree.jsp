...
<script type="text/javascript">
	...
	function loadSave(){
	   ...
	   $("#inputForm").attr("action","${ctx}/pbc/pbcPaymentAccountTwo/view/two/load?method=true&feedBackId="+feedBackId).submit();
	   // $("#inputForm").attr("action",$("#inputForm").attr("action")+"/load?feedBackId="+feedBackId);
	...
	function saveTwo(){
		...
		if(redioValue=="Y"){
			$("#inputForm").attr("action","${ctx}/pbc/pbcPaymentAccountThree/view/three/save?redioValue=Y&feedBackId="+feedBackId).submit();
		}else{
			$("#inputForm").attr("action","${ctx}/pbc/pbcPaymentAccountThree/view/three/save?feedBackId="+feedBackId).submit();
		}
	}
	...
	function page(n,s){
		...
		if(viewLook!=''){
			...
			$("#searchForm").attr("action","${ctx}/pbc/pbcPaymentAccountThree/view/three?no=true&pageNo="+n);   
		}else{
			
			$("#searchForm").attr("action","${ctx}/pbc/pbcPaymentAccountThree/view/three?pageNo="+n);
		} 
		$("#searchForm").submit();
    	return false;
    }
...
<body>
	...
	<form:form id="inputForm" modelAttribute="pbcPaymentAccount" action="${ctx}/pbc/pbcPaymentAccountThree/view/three/save" method="post" class="form-horizontal">
	<sys:message content="${message}"/>
		...
		<div class="control-group">	
			...
			<div class="control-right">
				...
				<div class="controls">
					<form:input path="feedBackOrgName" value="${pbcPaymentAccount.feedBackOrgName}" htmlEscape="false" maxlength="50" class="required" />
		...
		<div class="control-group">
			...
			<div class="control-right">
				...
				<div class="controls">
					<form:input path="operatorPhoneNumber"  value="${pbcPaymentAccount.operatorPhoneNumber}" htmlEscape="false" maxlength="12" class="required" />
					<span class="help-inline"><font color="red">*</font> </span>
				</div>
			</div>
		</div>
		
		<%-- <c:if test="${pbcPaymentAccount.yes=='Y'}">
			<div class="control-group">
				<div class="control-left">
	 ...
	 <div class="control-group">
		<div class="control-left">
			...
			<div class="controls">
				<form:input path="feedBackRemark"  value="${pbcPaymentAccount.feedBackRemark}" htmlEscape="false" maxlength="12" class="required" />
				<span class="help-inline"><font color="red">*</font> </span>
			</div>
		</div>
		<div class="control-right">
				...
				<div class="controls">
					<form:input path="abnormalDescribe" value="${pbcPaymentAccount.abnormalDescribe}" htmlEscape="false" maxlength="12" class="required" />
					<span class="help-inline"><font color="red">*</font> </span>
				</div>
			</div>
		...
		<%-- <c:if test="${pbcPaymentAccount.yes=='Y'}">
			<div class="control-right">
				<label class="control-label">上报失败原因：</label>
					<div class="controls">
							<form:input path="failRemark" id="failRemark" value="${pbcPaymentAccount.failRemark}" readOnly="true" style="border:0px;background-color:#fff;padding-top: 3px;"/>
							<span style="visibility: hidden;" class="help-inline"><font color="red">*</font> </span>
						</div>
					</div>
					<div class="control-right">
						...
						<div class="controls">
						<form:input path="riskRemark" id="riskRemark" value="${pbcPaymentAccount.riskRemark}" readOnly="true" style="border:0px;background-color:#fff;padding-top: 3px;"/>
						<span style="visibility: hidden;" class="help-inline"><font color="red">*</font> </span>
					</div>
			</div>
		</c:if> --%>
	...
	<table id="contentTable" class="table table-striped table-bordered table-condensed">
		...
		<c:forEach items="${page.list}" var="pbcPaymentAccount">
				<tr>
					<td>${pbcPaymentAccount.accountNumber}</td>
					...
					<%-- <td><fmt:formatDate value="${pbcPaymentAccount.successTime}" type="both" pattern="yyyy-MM-dd HH:mm:ss"/></td> --%>
					<c:if test="${empty ngp}">
						<td>
							<a href="${ctx}/pbc/pbcPaymentAccount/view/add?local=three&paymentAccountId=${pbcPaymentAccount.accountNumber}&applicationId=${applicationIdLoad}&feedBackId=${feedBackId}">添加银行卡信息</a>|
							<a href="${ctx}/pbc/pbcPaymentAccountTwo/view/two/add?local=three&paymentAccountId=${pbcPaymentAccount.accountNumber}&applicationId=${applicationIdLoad}&feedBackId=${feedBackId}">添加交易信息</a>
						</td>
					</c:if>
				</tr>
	...
	<c:if test="${empty pbcPaymentAccount.no}">
		<form id="loadForm" action="${ctx}/pbc/pbcPaymentAccountThree/view/three/loadModel?applicationId=${pbcPaymentAccount.applicationId}&feedBackId=${feedBackId}" method="post" enctype="multipart/form-data" class="form-horizontal">
			<div class="control-group" style="margin-top:30px">
				<div class="control-left">
