...
<body>
	<ul class="nav nav-tabs">
		<li class="active"><a href="${ctx}/settle/settleChannelRecordHis/">通道结算历史列表</a></li>
	</ul>
	<form:form id="searchForm" modelAttribute="settleChannelRecordHis" action="${ctx}/settle/settleChannelRecordHis/" method="post" class="breadcrumb form-search">
	...
	<table id="contentTable" class="table table-striped table-bordered table-condensed">
		...
		<tbody>
		<c:forEach items="${page.list}" var="settleChannelRecordHis">
			<tr>
				<td>${settleChannelRecordHis.channelName}</td>
				<td>${settleChannelRecordHis.channelType}</td>
				<td>${settleChannelRecordHis.payNum}</td>
				<td>
					<fmt:formatNumber value="${settleChannelRecordHis.totalAmount}" pattern="￥0.0000" />
				</td>
				<td><fmt:formatDate value="${settleChannelRecordHis.settleTime}" type="both" pattern="yyyy-MM-dd"/></td>
				<td>${settleChannelRecordHis.settleBath}</td>
				<td>
					<fmt:formatNumber value="${settleChannelRecordHis.costAmount}" pattern="￥0.0000" />
				</td>
				<td <c:choose>
			     		<c:when test="${settleChannelRecordHis.settleStatus=='已结算'}">
			     		 style="background:#6cf683" 	
			     		</c:when>
			     		<c:when test="${settleChannelRecordHis.settleStatus=='未结算'}">
			     		 style="background:#707c9b" 
			     		</c:when>
			     		<c:when test="${settleChannelRecordHis.settleStatus=='结算中'}">
			     		 style="background:#ffb300" 
			     		</c:when>
			     	</c:choose>>${settleChannelRecordHis.settleStatus}</td>
				<td>
					<a href="${ctx}/settle/settleChannelRecordHis/toDetail?settleBath=${settleChannelRecordHis.settleBath}">查看明细</a>
				</td>
			</tr>
