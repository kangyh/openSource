	...
	<script type="text/javascript">
		...
		var validateFrom = {
			validate: function(form){
				...
				var spreadId = $("#spreadId_q").val().trim();
		...
		function onExport(){
	        ...
	        $("#searchForm").attr("action","${ctx}/prom/promSaleOrder/view/export");
			validateFrom.validate($("#searchForm"));
		...
		function checkDelete() {
			...
			if(x==0) {
				...
				loading('正在提交，请稍等...');
				$.ajax({
					url:"${ctx}/prom/promSaleOrder/view/deleteManyData",
					type:"post",
					cache:false,
					timeout:10000,
					data:{
...
<body>
	<ul class="nav nav-tabs">
		<li class="active"><a href="${ctx}/prom/promSaleOrder/view">销售单查询</a></li>
		<shiro:hasPermission name="prom:promSaleOrder:edit"><li><a href="${ctx}/prom/promSaleOrder/view/initGenSaleOrder">销售单生成</a></li></shiro:hasPermission>
	</ul>
	<form:form id="searchForm" modelAttribute="promSaleOrder" action="${ctx}/prom/promSaleOrder/view" method="post" class="breadcrumb form-search">
		...
		<ul class="ul-form">
			...
			<li><label>分润时间：</label>
				<input id="beginDate" name="beginDate" type="text" readonly="readonly" class="input-medium Wdate"
					value="<fmt:formatDate value="${promSaleOrder.beginDate}" pattern="yyyy-MM-dd HH:mm:ss"/>"
				...
				<input id="endDate" name="endDate" type="text" readonly="readonly" class="input-medium Wdate"
					value="<fmt:formatDate value="${promSaleOrder.endDate}" pattern="yyyy-MM-dd HH:mm:ss"/>"
		...
		<ul class="ul-form">
			...
			<li><label>推广位编号：</label>
				<form:input id="spreadId_q" path="spreadId"  htmlEscape="false"  style="width:256px;" class="input-medium required" maxlength="32"/>
			</li>
			<li><label>商户编号：</label>
	...
	<table id="contentTable" class="table table-striped table-bordered table-condensed">
		...
		<tbody>
			<c:forEach items="${page.list}" var="psOrder" varStatus="seq">
				<tr id="tr_${psOrder.saleId}">
					<td>${seq.index+1}</td>
					<td><input type="checkbox" name="saleId" value="${psOrder.saleId}" class="exportParam"/></td>
					<td>${psOrder.orderId}</td>
					<td>${psOrder.warrantyId}</td>
					<td>${psOrder.orderBatch}</td>
					<td>${psOrder.spreadId}</td>
					<td>${psOrder.productId}</td>
					<td>${psOrder.productName}</td>
	...
	<div class="form-actions">
			...
			<form id="deleteForm" action="${ctx}/prom/promSaleOrder/view/deleteManyData" method="post"  >
				<input type="hidden" name="saleIds" id="saleIds_delete" value=""/>
